...
Le dépôt est scindé en **`backend/`** (tout le Python : `manage.py`, `config/`, `apps/`, `locale/`,
`requirements/`, `data/`) et **`frontend/`** (tout le visuel : `templates/` avec un dossier par app,
`static/` : CSS, JS, images, polices, vidéos). Le site reste rendu par Django : `settings.FRONTEND_DIR`
désigne `frontend/`. Les commandes Python se lancent depuis `backend/`. La racine ne contient que ces
deux dossiers, `Dockerfile` et `docker-compose.yml` (imposés par Docker), plus `.github/` (CI, README)
et `.claude/` (ce fichier). `backend/` héberge aussi `deploy/`, `docs/` et `sources/` (matériaux bruts de l'ancien site,
jamais dans l'image Docker).

Découpage en **apps métier autonomes** sous `backend/apps/` (un domaine = une app) :
...
[`backend/docs/architecture-navigation.md`](backend/docs/architecture-navigation.md). Elle
...
3. **Présentation** — tokens CSS dans `frontend/static/css/design-system.css`.
...
- L'arborescence cible de migration est formalisée dans
  `backend/docs/architecture-navigation.md`.
...
3. Écrire les scripts de reprise de contenu depuis Drupal 7 avec les slugs
  de `backend/docs/architecture-navigation.md`.
