...
# Usage : bash deploy/https.sh refonte.portdakar.sn admin@exemple.sn
set -euo pipefail
...
sed "s/__DOMAIN__/${DOMAIN}/g" deploy/nginx-https.conf > deploy/nginx-https.generated.conf
...
echo "  0 3 * * * cd $(pwd) && bash deploy/renew.sh >> /var/log/pad-renew.log 2>&1"
